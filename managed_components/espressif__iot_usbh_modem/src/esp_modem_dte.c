...
typedef struct esp_modem_dte_s {
    ...
    usb_modem_diag_t diag;                       /*!< Latest USB modem diagnostic snapshot */
} esp_modem_dte_t;

static bool modem_config_has_interface(const usb_config_desc_t *config_desc, int itf_num)
{
    if (config_desc == NULL || itf_num < 0) {
        return false;
    }

    usb_device_match_id_t id = {
        .match_flags = USB_DEVICE_ID_MATCH_INT_NUMBER,
        .bInterfaceNumber = itf_num,
    };
    return usbh_match_interface_in_configuration(config_desc, &id, NULL) != 0;
}
...
static const usb_modem_id_t *modem_device_match(const usb_modem_id_t *list, const usb_device_desc_t *device_desc, const usb_config_desc_t *config_desc)
{
    ...
    while (cur->match_id.match_flags != 0) {
        ...
        if (!modem_config_has_interface(config_desc, cur->modem_itf_num)) {
            goto _skip; // Modem interface does not exist in current configuration
        }
        if (cur->at_itf_num >= 0 && !modem_config_has_interface(config_desc, cur->at_itf_num)) {
            goto _skip; // AT interface does not exist in current configuration
        }
...
static void _usbh_dte_dev_event_cb(usbh_cdc_device_event_t event, usbh_cdc_device_event_data_t *event_data, void *user_ctx)
{
    ...
    case CDC_HOST_DEVICE_EVENT_CONNECTED: {
        ...
        dte->diag.device_seen = true;
        dte->diag.dev_addr = event_data->new_dev.dev_addr;
        dte->diag.vid = device_desc->idVendor;
        dte->diag.pid = device_desc->idProduct;
        dte->diag.bcd_device = device_desc->bcdDevice;
        dte->diag.device_class = device_desc->bDeviceClass;
        dte->diag.device_subclass = device_desc->bDeviceSubClass;
        dte->diag.device_protocol = device_desc->bDeviceProtocol;
        dte->diag.config_value = config_desc->bConfigurationValue;
        dte->diag.config_interfaces = config_desc->bNumInterfaces;
        dte->diag.cdc_matched_intf_num = event_data->new_dev.matched_intf_num;
        ...
        if (matched) {
            dte->diag.id_matched = true;
            dte->diag.matched_name = matched->name;
            dte->diag.modem_itf_num = matched->modem_itf_num;
            dte->diag.at_itf_num = matched->at_itf_num;
            dte->diag.modem_itf_present = modem_config_has_interface(config_desc, matched->modem_itf_num);
            dte->diag.at_itf_present = matched->at_itf_num >= 0 && modem_config_has_interface(config_desc, matched->at_itf_num);
    ...
    case CDC_HOST_DEVICE_EVENT_DISCONNECTED: {
        ...
        dte->cdc_port[USB_MODEM_CDC_INTF_MODEM] = NULL;
        dte->cdc_port[USB_MODEM_CDC_INTF_AT] = NULL;
        dte->diag.dte_connected = false;
        dte->diag.modem_port_open = false;
        dte->diag.at_port_open = false;
...
static esp_err_t usbh_ppp_init(iot_eth_driver_t *handle)
{
    ...
    memset(&dte->diag, 0, sizeof(dte->diag));
    dte->diag.at_itf_num = -1;
    dte->diag.modem_itf_num = -1;
    dte->diag.dev_addr = 0xFF;
...
bool esp_modem_dte_get_diag(iot_eth_driver_t *dte_drv, usb_modem_diag_t *diag)
{
    if (dte_drv == NULL || diag == NULL) {
        return false;
    }
    esp_modem_dte_t *dte = __containerof(dte_drv, esp_modem_dte_t, eth_driver);
    *diag = dte->diag;
    diag->modem_port_open = dte->diag.modem_port_open;
    diag->at_port_open = dte->diag.at_port_open;
    diag->dte_connected = dte->connect_status != 0;
    return true;
}
